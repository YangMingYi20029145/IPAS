...
1.在計算機安全中，自由選定存取控制由《可信計算機系統評估準則》所定義的存取控制中的一種類型。
2.它是根據主體的身份和他所屬的組限制對客體的訪問。所謂的自主，是因為擁有訪問權限的主體，可以直接地將訪問權限賦予其他主體。 
3.自由選定存取控制常常與強制存取控制對比

強制性存取控制（Mandatory Access Control，MAC）
1.強制存取控制在電腦安全領域指一種由作業系統約束的存取控制，目標是限制主體或發起者存取或對物件或目標執行某種操作的能力。
2.在實踐中，主體通常是一個行程或執行緒，物件可能是檔案、目錄、TCP/UDP埠、共享記憶體段、I/O裝置等。
3.主體和物件各自具有一組安全屬性

角色存取控制（Role-based Access Control，RBAC） 
1.以角色為基礎的存取控制，是資訊安全領域中，一種較新且廣為使用的存取控制機制，
2.其不同於強制存取控制以及自由選定存取控制直接賦予使用者權限，而是將權限賦予角色。
3.1996年，萊威·桑度等人在前人的理論基礎上，提出以角色為基礎的存取控制模型，故該模型又被稱為RBAC96

屬性存取控制（Attribute-based Access Control，ABAC） 
1.是一種透過 Attribute (屬性) 為依據的權限控管策略。
2.AWS 上提供 Tags (標籤) 的功能，來讓使用者們幫使用資源增加識別性、做 Cost allocation…等辨識需求。
3.甚至可以搭配 IAM Policy 來要求 IAM User 在建立 AWS Resources 時必須同時新增 Tags。
